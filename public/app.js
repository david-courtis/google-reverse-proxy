...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('geminiSettings', window.Components.geminiSettings);
    ...
    Alpine.data('app', () => ({
        ...
        startAutoRefresh() {
            ...
            // Setup visibility change listener (only once): refresh immediately
            // when the tab regains focus so the user always sees fresh numbers.
            if (!this._refreshVisibilitySetup) {
                this._refreshVisibilitySetup = true;
                document.addEventListener('visibilitychange', () => {
                    if (!document.hidden) Alpine.store('data').fetchData();
                });
            }
            ...
            if (interval > 0) {
                this.refreshTimer = setInterval(() => {
                    // Only poll quota while the tab is visible/focused
                    if (!document.hidden) Alpine.store('data').fetchData();
                }, interval * 1000);
            }
        },
