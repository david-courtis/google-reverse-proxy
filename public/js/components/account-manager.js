...
window.Components.accountManager = () => ({
    ...
    async onboardAccount(acc) {
        const store = Alpine.store('global');
        this.onboarding = acc.id;
        try {
            store.showToast('Onboarding ' + window.Redact.email(acc.email) + '… this can take up to a minute', 'info');
            const { response, newPassword } = await window.utils.request(
                '/v1/auth/onboard?id=' + encodeURIComponent(acc.id),
                {},
                store.webuiPassword
            );
            ...
            if (data.status === 'ok') {
                store.showToast('Onboarded ' + window.Redact.email(acc.email) + ', quota now available', 'success');
                Alpine.store('data').fetchData();
            } else if (data.status === 'banned' || data.status === 'verify') {
                store.showToast(data.error || ('Account ' + data.status), 'error');
                Alpine.store('data').fetchData();
            } else {
                throw new Error(data.error || 'Onboarding failed');
            }
        } catch (e) {
            store.showToast('Onboarding failed: ' + e.message, 'error');
        } finally {
            this.onboarding = '';
        }
    },
    ...
    async refreshAccount(id) {
        return await window.ErrorHandler.withLoading(async () => {
            const store = Alpine.store('global');
            const account = (Alpine.store('data').accounts || []).find(a => a.id === id);
            const displayEmail = account?.email || id;
            store.showToast(store.t('refreshingAccount', { email: Redact.email(displayEmail) }), 'info');
            ...
            const data = await response.json();
            if (data.status === 'ok') {
                store.showToast(store.t('refreshedAccount', { email: Redact.email(displayEmail) }), 'success');
                Alpine.store('data').fetchData();
            } else {
    ...
    async toggleAccount(id, enabled) {
        ...
        try {
            ...
            if (data.status === 'ok') {
                ...
                store.showToast(store.t('accountToggled', { email: Redact.email(displayEmail), status }), 'success');
    ...
    async executeDelete() {
        ...
        return await window.ErrorHandler.withLoading(async () => {
            ...
            const data = await response.json();
            if (data.status === 'ok') {
                store.showToast(store.t('deletedAccount', { email: Redact.email(displayEmail) }), 'success');
                Alpine.store('data').fetchData();
    ...
    async exportAccounts() {
        ...
        try {
            ...
            const payload = Array.isArray(data) ? { accounts: data, apiKeys: [] } : data;
            const count = (payload.accounts?.length || 0) + (payload.apiKeys?.length || 0);
            const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `google-reverse-proxy-accounts-${new Date().toISOString().split('T')[0]}.json`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);

            store.showToast(store.t('exportSuccess', { count }), 'success');
        } catch (e) {
